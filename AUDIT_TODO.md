...
- [x] 🟠 **Replace pos/wsd/ner/logging direct repo imports with query ports** → [ARCHITECTURE_AUDIT.md A-002, A-005](./ARCHITECTURE_AUDIT.md) · Effort: 2 days — Two outbound query ports in `genesis-common.port` (`DocumentQueryPort.workspaceIdForDocument`, `TokenQueryPort.documentIdForToken/formForToken/countTokensForDocument`) with adapters in `genesis-api` (`DocumentQueryAdapter`/`TokenQueryAdapter`, same composition-root pattern as A-004/A-007). All 8 services migrated: `PosTaggingService`/`NerAnnotationService`/`WsdAnnotationService` use the ports for document→workspace + token reads; their definition/sense/export/log siblings drop the duplicated `requireMember`/`requireAdmin` (+ `WorkspaceMemberRepository`/`WorkspaceRepository`/`MemberRole` reaches) in favor of the sanctioned `WorkspaceAccessControl`. The pos/ner tag-definition owner-bypass was dropped in favor of `requireAdmin`; this is safe because the owner is created as an ADMIN member and both `updateMemberRole` *and* (newly) `removeMember` now forbid changing/removing the owner's membership — adversarial review caught that `removeMember` previously had no owner guard, so an admin could orphan the owner; that guard was added here to make the "owner is always an ADMIN member" invariant actually enforced. Behavior preserved; 7 affected unit tests rewritten to mock the ports/WAC, plus new `DocumentQueryAdapterTest`/`TokenQueryAdapterTest` (8 cases) and a `removeMember`-owner-guard test. Zero foreign repository/entity imports remain in the four modules' `src/main`. **ArchUnit baseline ratcheted 174 → 80** (94 cross-module repo/entity reaches pruned). Note: two `TokenEntity` references survive in `WsdAnnotationRepository` JPQL `@Query` strings — not bytecode deps, not flagged by ArchUnit, out of scope.
